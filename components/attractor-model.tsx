...
export function AttractorModel({
  ...
  useEffect(() => {
    ...
    attractorConfig.calculate(positions, colors, attractorParams, iterations, false, {
      bass: 0,
      mid: 0,
      high: 0,
      volume: 0,
  ...
  const surfaceParticles = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const targetPos = new THREE.Vector3(ax * scaleFactor, ay * scaleFactor, az * scaleFactor)
  ...
  }, [particleCount, scale, rotationX, rotationY, rotationZ])
  ...
  return (
    <group ref={groupRef} scale={scale} rotation={[rotationX, rotationY, rotationZ]}>
      {/* Render the model mesh */}
      <primitive object={scene} scale={1}>
        <meshStandardMaterial color={0xffffff} transparent opacity={modelOpacity} metalness={0.3} roughness={0.7} />
      </primitive>
