...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return <div class={cn("font-medium", labelClassName)}>{labelFormatter(value, payload)}</div>
    }
...
function ChartLegendContent({
  ...
  return (
    <div class={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((item) => {
